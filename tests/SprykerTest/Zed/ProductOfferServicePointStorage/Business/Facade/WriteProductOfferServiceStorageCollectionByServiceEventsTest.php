 ...
 * @group Business
 * @group Facade
 * @group WriteProductOfferServiceStorageCollectionByServiceEventsTest
 * Add your own group annotations below this line
 */
class WriteProductOfferServiceStorageCollectionByServiceEventsTest extends Unit
{
     ...
     * @var string
     */
    protected const FAKE_SERVICE_ID = -1;
    ...
    public function testWriteProductOfferServiceStorageCollectionByServiceEvents(
        ...
        // Arrange
        [$productOfferTransfer, , $serviceTransfers] = $this->tester->createDataForProductServicePointPublishing(
        ...
        foreach ($serviceTransfers as $serviceTransfer) {
            $eventEntityTransfers[] = (new EventEntityTransfer())->setId($serviceTransfer->getIdServiceOrFail());
        }

        // Act
        $this->tester->getFacade()->writeProductOfferServiceStorageCollectionByServiceEvents($eventEntityTransfers);
    ...
    public function testShouldNotPublishWhenServiceDoesNotExist(): void
    {
        ...
        $eventTransfers = [
            (new EventEntityTransfer())->setId(static::FAKE_SERVICE_ID),
        ];
    ...
    public function testShouldPublishByChunks(): void
    {
        ...
        $eventEntityTransfers = [
            (new EventEntityTransfer())->setId($serviceTransfer->getIdServiceOrFail()),
            (new EventEntityTransfer())->setId($secondServiceTransfer->getIdServiceOrFail()),
        ];
